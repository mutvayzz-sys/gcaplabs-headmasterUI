...
├─────────────────────────────────────────────────────────────┤
│ Step 2: 配置 Token                                          │
│   Headmaster 设置页面 → 粘贴 Token → 验证 → 保存               │
├─────────────────────────────────────────────────────────────┤
│ Step 3: 启动 Bot                                            │
│   开启开关 → Bot 开始监听                                   │
├─────────────────────────────────────────────────────────────┤
...
| -------------- | ---------------------------------------- |
| 配对码认证     | 6位随机码，10分钟有效                    |
| 本地批准       | 必须在 Headmaster 中批准，非 Telegram 中 |
| 用户白名单     | 仅授权用户可使用                         |
| 速率限制       | 防止滥用                                 |
...
├─────────────────────────────────────────────────────────────┤
│ Step 4: 配置凭证                                            │
│   Headmaster 设置页面 → 粘贴 App ID、App Secret → 验证 → 保存   │
├─────────────────────────────────────────────────────────────┤
│ Step 5: 启动 Bot                                            │
...
与 Telegram 相同，采用本地批准模式。配对码通过 Lark 消息发送给用户，用户在 Headmaster 中批准。
...
| 会话模式 | 单活跃会话                 |
| 新建会话 | 点击 🆕 按钮清空上下文     |
| 会话存储 | 独立于 Headmaster GUI 会话 |
| Agent    | 固定 Gemini                |
| Model    | 使用 Headmaster 默认配置   |
...
| Model 切换 | 支持动态选择模型                       |
| 会话同步   | Telegram 会话与 Headmaster 会话关联    |
