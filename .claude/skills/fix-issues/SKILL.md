...
```
~/.headmaster-fix-issues/skip-list.json
```
...
```bash
gh api repos/iOfficeAI/Headmaster/issues/<number>/comments \
...
```bash
gh issue list --repo iOfficeAI/Headmaster --state open --label bug --limit 50 \
...
gh api repos/iOfficeAI/Headmaster/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo iOfficeAI/Headmaster --body "$(cat <<'EOF'
...
```bash
gh pr list --repo iOfficeAI/Headmaster --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.headmaster-fix-issues/skip-list.json`
