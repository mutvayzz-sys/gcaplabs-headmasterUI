...
|---|---|---|---|
| `headmaster-desktop/` | **The actual app.** Electron + Vite + React + TypeScript. This is the build target. Pushed to `gcaplabs-headmasterUI` (private) on GitHub. | Always — unless explicitly told otherwise. | Never. |
...
| `_support/upstream/aionui/` | The original Apache-2.0 source repo (`iOfficeAI/AionUi`). This is **what `headmaster-desktop/` is forked from**. | When the user asks "what was the original code" or to compare. | For active build work, use `headmaster-desktop/`. |
...
**One product**: Headmaster Desktop (Electron). Its source is `headmaster-desktop/`. Everything else is upstream / reference / scratch.

**The build target:** run from `headmaster-desktop/`. Build commands:
...
Output lands in `headmaster-desktop/out/`:
...
1. **Read** `headmaster-desktop/docs/white-label/HEADMASTER-VOCABULARY.csv` before naming anything user-facing. **Do not** ship AionUi / aionui / aionrs / Cowork / Hermes in the UI. Use the Headmaster column.
2. **Read** `headmaster-desktop/docs/white-label/WHITE-LABEL-AUDIT.md` before touching anything in `headmaster-desktop/packages/desktop/src/process/`. `HERMES_DESKTOP_*` env vars and the `hermes-media://` protocol are off-limits.
3. **Edit** in `headmaster-desktop/` only. Do not edit `runtime/hermes-agent/`, `_support/upstream/`, or `_support/staging/`. Those are reference clones.
...
- **`gcaplabs-site/` is the website, not the desktop.** Different stack, different product surface. If the user says "change the website", that's `gcaplabs-site/`. If they say "change the app", that's `headmaster-desktop/`.
...
- **White-label audit:** `headmaster-desktop/docs/white-label/WHITE-LABEL-AUDIT.md` (read before any rename work).
- **Vocab mapping:** `headmaster-desktop/docs/white-label/HEADMASTER-VOCABULARY.csv` (read before any naming work).
- **Hermes runtime recon:** `runtime-recon/RECON.md` (read before any runtime/backend work).
- **The plan:** `headmaster-desktop/docs/white-label/WHAT-WE-TAKE.md` (high-level; likely out of date, the recon wins).
- **Active backlog:** `headmaster-desktop/todo.md` — read this first when resuming desktop work. **All 16 TODO items completed 2026-06-18** (settings restructure, agent scanner, gateway checker, update checker, memory embed, feature removals, white-label audit). Remaining: build + smoke test, DeepWiki update, Honcho upgrade.
...
- **Last few things we did** (most recent first):
  ...
  7. Folder restructure: `Desktop/Gcaplabs.com/` → `Desktop/GCAP-Labs/`, with `headmaster-desktop/`, `headmaster-hub/`, `gcaplabs-site/`, `runtime/`, `runtime-recon/`, `_support/upstream/`, `_support/staging/`, `_support/archive/`.
