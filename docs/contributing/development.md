...
git clone https://github.com/iOfficeAI/Headmaster.git
cd Headmaster
...
When you have two clones of the repository (e.g. `Headmaster` and `Headmaster-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `HEADMASTER_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`Headmaster-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.headmaster-dev-2`, `~/.headmaster-config-dev-2`)
...
Headmaster uses **electron-vite** for fast bundling:
