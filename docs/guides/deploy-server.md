# Headmaster Headless Server Deployment Guide

Deploy Headmaster WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- Headmaster `.deb` package from [Releases](https://github.com/iOfficeAI/Headmaster/releases)
...
wget https://github.com/iOfficeAI/Headmaster/releases/latest/download/Headmaster-linux-amd64.deb

# Install
sudo dpkg -i Headmaster-linux-amd64.deb
...
Headmaster is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/Headmaster/start-headmaster.sh`:

```bash
#!/bin/bash
# Headmaster WebUI headless startup script
# Usage: ./start-headmaster.sh [start|stop|restart|status]

PIDFILE="/var/run/headmaster.pid"
LOGFILE="/var/log/headmaster.log"
WORKDIR="$HOME"  # Change to your workspace directory

start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/Headmaster --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "Headmaster failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "Headmaster is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping Headmaster (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "Headmaster --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "Headmaster stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "Headmaster is not running."
...
```bash
chmod +x /opt/Headmaster/start-headmaster.sh
```

> **Tip**: `WORKDIR` determines the directory Headmaster can access for file operations. Set it to your project workspace.

---

## Remote Access

Headmaster WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for Headmaster (Electron / Chromium Layer)
...
Create `/opt/Headmaster/proxy.pac`:
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/Headmaster --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/Headmaster/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: Headmaster Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside Headmaster WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by Headmaster's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i Headmaster-linux-amd64.deb`             |
| Headmaster can only access `/tmp`         | Set `WORKDIR` in the startup script to your workspace path   |
...
# Headmaster 无头服务器部署指南

在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 Headmaster WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- Headmaster `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/Headmaster/releases)）
...
Headmaster 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/Headmaster/start-headmaster.sh`：

```bash
#!/bin/bash
# Headmaster WebUI 无头启动脚本
# 用法: ./start-headmaster.sh [start|stop|restart|status]
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 Headmaster WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "Headmaster 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "Headmaster 未在运行"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "正在停止 Headmaster (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "Headmaster --webui" 2>/dev/null
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "Headmaster 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "Headmaster 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
Headmaster WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（Headmaster Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/Headmaster/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/Headmaster/proxy.pac"`
...
### 第四步：Headmaster 内置代理（Gemini API）
...
| Headmaster 只能访问 /tmp | 修改启动脚本中的 `WORKDIR`            |
