...
Headmaster supports CDP for external debugging tools integration. In development mode (`just dev`), CDP is enabled by default on port 9230.

## Enable CDP in Production

1. Open Headmaster Settings → System → Developer Debug
...
Headmaster can integrate with other MCP tools for enhanced development experience:
...
Once configured, you can use MCP tools to interact with Headmaster:

- `list_pages` — List all open pages in Headmaster
...
Headmaster 支持 CDP 用于外部调试工具集成。在开发模式 (`just dev`) 下，CDP 默认在端口 9230 启用。

## 在生产环境启用 CDP

1. 打开 Headmaster 设置 → 系统 → 开发者调试
2. 启用"启用远程调试 (CDP)"
3. 重启应用
...
Headmaster 可以集成其他 MCP 工具来增强开发体验：
...
配置完成后，可以使用 MCP 工具与 Headmaster 交互：

- `list_pages` — 列出 Headmaster 中所有打开的页面
