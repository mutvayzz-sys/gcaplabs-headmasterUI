...
2. 页面顶部显示标题"Channels"、功能描述文本（"Connect Telegram, Lark, and DingTalk to interact with Headmaster from IM apps."），以及 2 步引导提示
...
**用户故事**：作为用户，我希望通过输入 Telegram Bot Token 连接 Telegram，并管理通过 Telegram 与 Headmaster 交互的授权用户。
...
**用户故事**：作为用户，我希望通过配置飞书应用凭据连接 Lark/飞书，与 Headmaster 进行 IM 对话。
...
**用户故事**：作为用户，我希望通过配置钉钉应用凭据连接 DingTalk，与 Headmaster 进行 IM 对话。
...
**用户故事**：作为用户，我希望通过配对码机制控制哪些 IM 用户可以与我的 Headmaster 交互，保证安全性。
...
2. 首次消息触发配对请求，Headmaster 通过 `channel.pairingRequested.on()` 实时推送到设置页面
