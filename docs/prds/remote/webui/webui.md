...
**用户故事**：作为桌面端用户，我希望通过一个开关快速启用或禁用 WebUI 服务，以便在需要时通过浏览器远程访问 Headmaster。
...
端口解析（覆盖式优先级）：CLI 参数（`--port` / `--webui-port`）> 环境变量（`HEADMASTER_PORT` / `PORT`）> 配置文件（`userData/webui.config.json`）> 默认值（25808/25809）

远程访问（多源 OR 聚合 — 只要任一来源为 true 即启用）：`isRemoteMode` || 环境变量（`HEADMASTER_ALLOW_REMOTE` / `HEADMASTER_REMOTE` / `HEADMASTER_HOST=0.0.0.0`）|| 配置文件（`allowRemote: true`）|| ConfigStorage 偏好
...
**用户故事**：作为用户，我希望控制是否允许局域网内其他设备访问 WebUI，以便用手机或其他电脑访问 Headmaster。
