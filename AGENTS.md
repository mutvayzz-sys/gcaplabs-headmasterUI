...
| `hermeshq/`                                      | **HermesHQ legacy backend** (FastAPI + PostgreSQL). Vendored from `mutvayzz-sys/gcaplabs-hermeshq` with `.git` removed. This is a historical / archive copy of the pre-Agent37 control plane. It is no longer the source of truth for any active desktop API or provision response. | Historical reference only. Do not use for new backend work; the active cloud path is Agent37. | Don't deploy from here. If you need to change the legacy backend for reference, push from `../gcaplabs-hermeshq` instead. |
