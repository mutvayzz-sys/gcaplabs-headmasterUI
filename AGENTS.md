...
| `gcaplabs-hermeshq/`                              | **HermesHQ backend** (FastAPI + PostgreSQL). The cloud control plane that provisions desktop clients. Contains provision endpoints, provider catalog, agent configs, container supervisor. Copied from `../gcaplabs-hermeshq` with `.git` removed. Source of truth for provision-response schema changes. | When modifying the provision endpoint, provider catalog, agent model config, or any backend API the desktop talks to. | Don't deploy from here — the live backend runs in Docker on the Mac Mini. Edit + push to the remote repo. |
