 ...
 * parsing the electron-log file for [Headmaster:ready] / [Headmaster:init] /
 * [Headmaster:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [Headmaster:ready] marks
  ...
  // Parsed from [Headmaster:init] marks
  initTotalMs: number;
  // Parsed from [Headmaster:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'Headmaster-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'Headmaster', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'Headmaster', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'Headmaster', 'logs', `${today}.log`));
  }
...
// Matches: [Headmaster:ready] <label> +<ms>ms
// Matches: [Headmaster:init]  <label> +<ms>ms
// Matches: [Headmaster:process] <label> +<ms>ms
const MARK_REGEX = /\[Headmaster:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[Headmaster] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[Headmaster] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[Headmaster] Showing main window')) marks.logs.showingMainWindow = true;
  }
...
async function launchApp(timeoutMs: number, withMemory: boolean): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      HEADMASTER_DISABLE_AUTO_UPDATE: '1',
      HEADMASTER_E2E_TEST: '1',
      HEADMASTER_DISABLE_DEVTOOLS: '1',
      HEADMASTER_CDP_PORT: '0',
      NODE_ENV: 'production',
    },
    timeout: timeoutMs,
  });
