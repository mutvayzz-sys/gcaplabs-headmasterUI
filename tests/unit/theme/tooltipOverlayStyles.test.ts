...
describe('arco tooltip and popover overlay styles', () => {
  it('defines shared light and dark overlay tokens for tooltip-like surfaces', () => {
    ...
    expect(css).toContain('--headmaster-overlay-bg: #ffffff;');
    expect(css).toContain('--headmaster-overlay-text: #1d2129;');
    expect(css).toContain("body[arco-theme='dark'] {");
    expect(css).toContain('--headmaster-overlay-bg: #0e0e0e;');
    expect(css).toContain('--headmaster-overlay-text: #f2f3f5;');
  });
  ...
  it('applies the shared overlay tokens to tooltip, popover, and popconfirm surfaces', () => {
    ...
    expect(css).toContain('background: var(--headmaster-overlay-bg) !important;');
    expect(css).toContain('color: var(--headmaster-overlay-text) !important;');
    expect(css).toContain('border: 1px solid var(--headmaster-overlay-border) !important;');
