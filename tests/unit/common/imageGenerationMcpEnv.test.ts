...
describe('resolveImageGenerationMcpEnv', () => {
  ...
  it('matches legacy env by platform, base URL, and model when provider id is absent', () => {
    const result = resolveImageGenerationMcpEnv(undefined, [geminiProvider], {
      HEADMASTER_IMG_PLATFORM: 'gemini',
      HEADMASTER_IMG_BASE_URL: 'https://generativelanguage.googleapis.com/',
      HEADMASTER_IMG_MODEL: 'gemini-3-pro-image-preview',
      HEADMASTER_IMG_API_KEY: 'stale-key',
    });

    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.source).toBe('field-match');
    expect(result.env.HEADMASTER_IMG_PROVIDER_ID).toBe('03c8482c');
    expect(result.env.HEADMASTER_IMG_API_KEY).toBe('provider-key');
  });
