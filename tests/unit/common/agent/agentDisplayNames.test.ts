...
describe('agentDisplayNames', () => {
  it('replaces forbidden upstream agent labels with Headmaster', () => {
    expect(sanitizeAgentDisplayName({ name: 'Aion CLI', backend: 'aionrs', agent_type: 'aionrs' })).toBe(
      'Headmaster'
    );
  ...
  it('keeps real third-party CLI product names', () => {
    expect(sanitizeAgentDisplayName({ name: 'Claude Code', backend: 'claude', agent_type: 'acp' })).toBe(
      'Claude Code'
    );
