...
describe('buildSendFailureError', () => {
  it('classifies 409 already-processing as HEADMASTER_CONVERSATION_BUSY (wait, not retry)', () => {
  ...
  it('falls back to HEADMASTER_INTERNAL_ERROR for non-conflict 409 (different message)', () => {
    ...
    expect(result.code).toBe('HEADMASTER_INTERNAL_ERROR');
    expect(result.retryable).toBe(true);
  });

  it('falls back to HEADMASTER_INTERNAL_ERROR for non-HTTP errors', () => {
