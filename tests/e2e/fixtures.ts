 ...
 *      (e.g. out/linux-unpacked/aionui, out/mac-arm64/Headmaster.app, out/win-unpacked/Headmaster.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/Headmaster.exe  or  out/win-x64-unpacked/Headmaster.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'Headmaster.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/Headmaster.app/Contents/MacOS/Headmaster  or  out/mac/Headmaster.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'Headmaster');
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['aionui', 'Headmaster']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    HEADMASTER_EXTENSIONS_PATH: process.env.HEADMASTER_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
