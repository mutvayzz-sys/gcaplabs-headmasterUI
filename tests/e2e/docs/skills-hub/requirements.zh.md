...
**核心定位**：SkillsHub（技能中心）是 Headmaster 的统一技能管理平台，提供一站式的技能导入、导出、删除、搜索与分类展示功能。用户通过该页面集中管理 AI agent 所需的所有技能，实现"一次安装，全助手通用"。
...
2. **Custom Skills**（自定义技能）：位于用户目录 `~/.headmaster/skills/`
...
4. 实现：`fsBridge.ts:1629-1676`
   ...
   - 目标路径：`~/.headmaster/skills/{skillName}`
...
- 用户添加错误路径后无法通过 UI 删除
- 必须手动编辑 `~/.headmaster/custom_external_skill_paths.json` 文件
...
```typescript
{
  userSkillsDir: string; // ~/.headmaster/skills
...
sequenceDiagram
    ...
    Bridge->>FS: 创建 symlink 到 ~/.headmaster/skills/{name}
...
**位置**：`~/.headmaster/`（`src/process/utils/initStorage.ts`）
...
**文件路径**：`~/.headmaster/custom_external_skill_paths.json`
...
- **解决方案**：手动编辑 `~/.headmaster/custom_external_skill_paths.json`
- **E2E 测试范围**：不包含
