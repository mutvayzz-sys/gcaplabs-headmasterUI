...
- **Builtin**：随 Headmaster 发布，不可删除，用户可导出到外部 CLI
- **Custom**：用户自行导入，可删除，可导出
- **Extension**：插件贡献，不可删除/导出（由插件管理）
- **Auto**：系统自动注入，无需用户选择（如 headmaster-skills）
...
3. **Extension Skills 数据来源**?
   - E2E 环境的 `HEADMASTER_EXTENSIONS_PATH` 已在 `fixtures.ts:112` 设置为 `examples/`
...
- 环境配置(`HEADMASTER_EXTENSIONS_PATH`)
- 临时目录创建策略
- 外部源构造方案
- My Skills 预置方案
- 数据隔离与清理策略
...
- ✅ **替代**:导入后验证 `skill.location` 路径格式(应包含 `~/.headmaster/skills/`)
...
- Extension Skills:依赖 `HEADMASTER_EXTENSIONS_PATH`(已在 fixtures.ts 配置)
...
- 4.2.1 目录结构 → 指导测试数据构造(在 `~/.headmaster/skills/` 创建测试 skill)
...
1. **环境配置**
   - `HEADMASTER_EXTENSIONS_PATH` 指向 `examples/`(已在 fixtures.ts 配置)
...
```typescript
// test-strategy § 3.2
const tempExternal = fs.mkdtempSync(path.join(os.tmpdir(), 'headmaster-e2e-external-'));

// test-cases § 3.2
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'e2e-external-'));
```

**细微差异**: 前缀 `headmaster-e2e-external-` vs `e2e-external-`
**建议**: 统一为 `headmaster-e2e-external-` (test-strategy 的更明确)
...
- test-strategy § 3.2: `headmaster-e2e-external-`
- test-cases § 3.2: `e2e-external-`

**影响**: 不影响功能，但统一命名可提升可读性和问题排查效率

**建议**: 在 test-strategy 或 test-cases 任一文档中明确统一为 `headmaster-e2e-external-`，并让双方在实现时遵循
...
```markdown
**注意**：E2E 测试依赖 `HEADMASTER_EXTENSIONS_PATH=examples/`（已在 `fixtures.ts:112` 配置）。
...
- `HEADMASTER_EXTENSIONS_PATH` 环境变量（已在 E2E fixtures 中配置为 `examples/`）
...
- `_builtin/` 目录存在且包含技能（如 `headmaster-skills`）
...
- `HEADMASTER_EXTENSIONS_PATH` 环境变量指向 `examples/`
...
- `_builtin/` 目录存在且包含至少 1 个技能（如 `headmaster-skills`）
...
✅ **前置条件明确**：`_builtin/` 目录存在（如 `headmaster-skills`）
...
- ✅ 显示至少 1 个自动技能卡片（如 `headmaster-skills`）
```
...
- Auto-injected skills 位于 `_builtin/` 目录，可选存在（如 `headmaster-skills`）
