...
- ✅ SQLite 数据库独立(`headmaster.db`)
- ✅ 扩展路径指向示例目录(`HEADMASTER_EXTENSIONS_PATH=examples/`)
...
// Returns:
// {
//   userSkillsDir: '~/.headmaster/skills/',     // My Skills 存储目录
...
- E2E 框架已设置 `HEADMASTER_EXTENSIONS_PATH=examples/`(`fixtures.ts:112`)
...
```typescript
// Setup: 创建临时目录
const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'headmaster-e2e-custom-'));
...
| `auto-skill-card-${normalizeTestId(skill.name)}`      | 自动注入技能卡片            | line ~520 `<div className="skill-card">`        | `auto-skill-card-headmaster-skills`       |
...
- 共享同一个 SQLite 数据库(`headmaster.db`)
- 并行会导致写锁冲突 + 测试数据污染
