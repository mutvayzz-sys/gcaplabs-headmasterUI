...
```typescript
return path.join(getDataPath(), 'headmaster.db');
```

**userData directory** (`src/process/utils/configureChromium.ts:18-26`):

- Dev mode: `~/Library/Application Support/Headmaster-Dev/` (macOS)
- Database: `{userData}/config/headmaster.db`
- Shared by all E2E tests

**Conflict scenario**: If Assistant tests and Skills tests run in parallel workers:

1. Both access same `headmaster.db` file
...
```typescript
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'headmaster-e2e-state-'));
...
```typescript
HEADMASTER_EXTENSION_STATES_FILE: process.env.HEADMASTER_EXTENSION_STATES_FILE || e2eStateFile;
```
...
**Critical bottleneck**: `workers: 1` enforced + shared `headmaster.db` → parallel execution impossible without refactoring.
...
1. Extend `HEADMASTER_E2E_TEST` to include worker ID:
   ```typescript
   HEADMASTER_E2E_TEST_WORKER_ID: process.env.PLAYWRIGHT_WORKER_INDEX || '0';
   ```
2. Modify `getDevAppName()` to return worker-specific name:
   ```typescript
   const workerId = process.env.HEADMASTER_E2E_TEST_WORKER_ID || '0';
   return `Headmaster-E2E-Worker-${workerId}`;
   ```
3. Each worker gets isolated:
   - `~/Library/Application Support/Headmaster-E2E-Worker-0/config/headmaster.db`
   - `~/Library/Application Support/Headmaster-E2E-Worker-1/config/headmaster.db`
...
**Complexity**: High (requires main process to read `HEADMASTER_DATABASE_PATH`, conflicts with userData convention)
