...
4. **后端持久化**
   - backend 独占 `headmaster.db`，Electron 不再直接访问 SQLite
...
- 解析顺序: 环境变量 `AION_CLI_PATH` → `~/.headmaster/bin/aion-<platform>-<arch>` → 系统 PATH 中的 `aion` 命令
...
| `aioncore headmaster.db`                                                      | —      | conversations + messages 由 backend 独占持久化                      |
