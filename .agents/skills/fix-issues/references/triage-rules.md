...
# Check open PRs
gh pr list --repo iOfficeAI/Headmaster --state open --search "<error-keyword>" --json number,title

# Check closed PRs (recently merged)
gh pr list --repo iOfficeAI/Headmaster --state merged --search "<error-keyword>" --limit 5 --json number,title,mergedAt
```
