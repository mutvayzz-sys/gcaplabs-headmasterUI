...
async function syncBuiltinMcpConfig(configFile: ConfigFile): Promise<void> {
  ...
  console.info(
    '[Headmaster] Synced builtin MCP config to backend settings (%d builtin servers)',
...
export async function runBackendMigrations(configFile: ConfigFile): Promise<void> {
  await CLEANUP_STEPS.reduce<Promise<void>>(async (previous, step) => {
    ...
    try {
      await step.run();
      console.info(`[Headmaster] Backend migration step completed: ${step.name} (${Date.now() - start}ms)`);
    } catch (error) {
      console.error(`[Headmaster] Backend migration step failed: ${step.name} (${Date.now() - start}ms)`, error);
    }
  }, Promise.resolve());
  ...
  await MIGRATION_STEPS.reduce<Promise<void>>(async (previous, step) => {
    ...
    try {
      ...
      if (!completed) {
        console.warn(`[Headmaster] Backend migration step incomplete: ${step.name} (${elapsed}ms)`);
        return;
      }
      console.info(`[Headmaster] Backend migration step completed: ${step.name} (${elapsed}ms)`);
    } catch (error) {
      const elapsed = Date.now() - start;
      console.error(`[Headmaster] Backend migration step failed: ${step.name} (${elapsed}ms)`, error);
    }
  }, Promise.resolve());
  ...
  try {
    ...
    console.info(`[Headmaster] Backend migration step completed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`);
  } catch (error) {
    console.error(`[Headmaster] Backend migration step failed: syncBuiltinMcpConfig (${Date.now() - syncStart}ms)`, error);
  }
}
