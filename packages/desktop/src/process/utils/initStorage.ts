...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[Headmaster] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[Headmaster] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[Headmaster] 数据迁移失败:', error);
  }
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[Headmaster] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
...
const initStorage = async () => {
  ...
  try {
    ...
    console.error('[Headmaster] Failed to ensure assistant dirs:', error);
  }
