...
// Override via HEADMASTER_CDP_PORT env variable. Set to "0" to disable.
//
...
// Registry file: ~/.headmaster-cdp-registry.json
// ---------------------------------------------------------------------------
...
function findAvailablePort(preferredPort: number): number {
  ...
  console.log(
    `[CDP] Port ${preferredPort} is occupied by another Headmaster instance, scanning range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END}`
  );
  ...
  console.warn(
    `[CDP] All ports in range ${CDP_PORT_RANGE_START}-${CDP_PORT_RANGE_END} are used by active Headmaster instances, trying ${preferredPort}`
  );
  return preferredPort;
}
...
function resolveCdpPortFromEnv(): number | null | undefined {
  const envVal = process.env.HEADMASTER_CDP_PORT;
