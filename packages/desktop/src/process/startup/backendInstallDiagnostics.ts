...
export function collectBackendInstallDiagnostics(
  ...
  const bundledDirPath = resourcesPath ? pathApi.join(resourcesPath, BUNDLED_HERMES_DIR) : undefined;
  const runtimeDirPath =
    resourcesPath && runtimeKey ? pathApi.join(resourcesPath, BUNDLED_HERMES_DIR, runtimeKey) : undefined;
  const binaryPath =
