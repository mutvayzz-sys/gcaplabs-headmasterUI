 ...
 * generate-qr-token) are NOT handled here — those are HTTP routes on Hermes's
 ...
 * WebUI instance requires Electron's app.* / Node child_process — Hermes
...
async function maybeSeedInitialPassword(): Promise<void> {
  const port = getBackendPort();
  if (!port) {
    throw new Error('[WebUI] Cannot start: Hermes is not running (globalThis.__backendPort unset)');
  }
