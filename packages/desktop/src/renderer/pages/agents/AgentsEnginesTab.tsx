...
import { ArrowCounterClockwise, Robot, ChatCircle, Code, MagnifyingGlass } from '@phosphor-icons/react';
...
const AgentsEnginesTab: React.FC = () => {
  ...
  return (
    <div className='flex flex-col h-full min-h-0'>
      <div className='flex items-center justify-end gap-8px mb-12px shrink-0'>
        <Button
          type='secondary'
          size='small'
      ...
      <div className='flex items-center gap-10px mb-16px shrink-0'>
        <Input
          ...
          value={search}
          onChange={setSearch}
          placeholder={t('agents.search', { defaultValue: 'Search engines...' })}
      ...
      <div className='flex-1 min-h-0 overflow-y-auto'>
        ...
        {!isLoading && !error && filtered.length === 0 && (
          <Empty description={t('agents.empty', { defaultValue: 'No engines found' })} />
        )}
            ...
            return (
              ...
              >
                <div className='flex items-center gap-10px'>
                  {logo ? (
                    <img src={logo} alt={`${engineLabel} logo`} className='w-36px h-36px object-contain rd-8px' />
                  ) : (
...
export default AgentsEnginesTab;
