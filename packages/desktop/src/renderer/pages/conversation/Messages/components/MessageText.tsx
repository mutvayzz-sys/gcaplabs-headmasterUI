...
import { HEADMASTER_FILES_MARKER } from '@/common/config/constants';
...
const parseFileMarker = (content: string) => {
  const markerIndex = content.indexOf(HEADMASTER_FILES_MARKER);
  if (markerIndex === -1) {
    return { text: content, files: [] as string[] };
  }
  const text = content.slice(0, markerIndex).trimEnd();
  const afterMarker = content.slice(markerIndex + HEADMASTER_FILES_MARKER.length).trim();
