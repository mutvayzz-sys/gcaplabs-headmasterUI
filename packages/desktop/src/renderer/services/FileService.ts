...
import { HEADMASTER_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理Headmaster时间戳后缀，返回原始文件名
export function cleanHeadmasterTimestamp(file_name: string): string {
  return file_name.replace(HEADMASTER_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(file_path: string): string {
  const file_name = file_path.split(/[\\/]/).pop() || '';
  return cleanHeadmasterTimestamp(file_name);
}
