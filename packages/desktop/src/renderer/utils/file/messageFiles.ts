import { HEADMASTER_FILES_MARKER, HEADMASTER_TIMESTAMP_REGEX } from '@/common/config/constants';
...
export const buildDisplayMessage = (input: string, files: string[], workspacePath: string): string => {
  ...
  const displayPaths = files.map((file_path) => {
    const sanitizedPath = file_path.replace(HEADMASTER_TIMESTAMP_REGEX, '$1');
    ...
    if (isAbsolute) {
      ...
      if (normalizedFile.startsWith(normalizedWorkspaceWithForwardSlash + '/')) {
        ...
        return `${normalizedWorkspace}/${relativePath.replace(HEADMASTER_TIMESTAMP_REGEX, '$1')}`;
      }
