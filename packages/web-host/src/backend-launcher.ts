...
export type BackendLaunchOptions = {
  ...
  logDir?: string;
  /**
   * System dirs exposed to the backend via HEADMASTER_{CACHE,WORK,LOG}_DIR env.
...
export function buildSpawnArgs(config: SpawnConfig): string[] {
  const logLevel = process.env.HEADMASTER_LOG_LEVEL || (config.isPackaged ? 'info' : 'debug');
  const args = [
    '--port',
    String(config.port),
    '--data-dir',
